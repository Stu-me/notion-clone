...
const reorderBlock = asyncHandler(async(req, res)=>{
    ...
    // validate input
    if(!Array.isArray(blocks) || blocks.length === 0){ // checks that blocks is array and not empty
        res.status(400);
...
module.exports = {getAllBlocks,createBlock,updateBlock,deleteBlock,reorderBlock}